...
function random(min, max) { // Рандом лоігійка для майбутнього заповнення
 ...
 class field_completion{
  ...
  constructor(height, width) {
      ...
      for (var i = 0; i < this.#height; i++) {
        for (var j = 0; j < this.#width; j++) {
          if (this.#arr_current[i][j] == 1) {
            // alive
            main_field_box.setContent(main_field_box.content + "  ".bgBlack);
          }
          else {
            //dead
    ...
    #define_point(height, width) { // Логіка життя(правила)
      //квадрат1 статична фігура
      this.#arr_current[1][1] = 1;
      this.#arr_current[1][2] = 1;
      this.#arr_current[2][1] = 1;
...
// Завершити Escape, q або Control-C.
...
button_3.on("press", function () {
  //почати
  if (input_1.value==1) speed=1000;
  else if (input_1.value==2) speed=500;
  else if (input_1.value==3) speed=300;
  if (input_1.value=='') speed=1000;
